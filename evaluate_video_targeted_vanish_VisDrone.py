...
def resolve_target_class_ids(names, target):
    # 사용자가 입력한 클래스명 또는 클래스 번호를 YOLO class id로 변환한다.
...
def filter_result_by_classes(result, class_ids):
    # 전체 탐지 결과 중 공격 대상으로 선택한 클래스만 남긴다.
...
def run_fastsam_prompt_mask(fastsam, img, target_boxes, device, size):
    # 선택 클래스 bbox를 FastSAM prompt로 사용해 객체 영역 마스크를 생성한다.
...
def build_target_mask(fastsam, img, original_result, class_ids, conf_thres, device, size):
    # FastSAM 마스크와 YOLO bbox 마스크의 교집합만 공격 영역으로 사용한다.
...
def build_targeted_vanish_image(img, target_mask, g_v):
    # Vanish 생성자 출력은 지정된 target_mask 영역에만 적용한다.
...
def clamp_linf_from_original(orig, adv, max_linf):
    # 반복 공격 시 원본 대비 최대 픽셀 변화량이 설정값을 넘지 않도록 제한한다.
    if max_linf <= 0:
        return adv
...
def run_feedback_vanish(
    ...
    # 공격 후에도 남은 target bbox가 있으면 같은 영역에 Vanish를 반복 적용한다.
    # 기본 max_steps=1은 기존 단일 적용 방식과 동일하다.
    adv = img
    ...
    for step_idx in range(max(1, max_steps)):
        ...
        if residual_mask_mode == "fastsam":
            # 더 정밀하지만 느린 방식: 남은 객체를 FastSAM으로 다시 분할한다.
            ...
            # 더 빠른 방식: 남은 target bbox 전체를 다음 공격 영역으로 사용한다.
...
def draw_target_highlights(img, result, class_ids):
    # 선택된 target class의 bbox를 노란색으로 강조해 시연 중 공격 대상을 명확히 표시한다.
    out = img.copy()
    if result.boxes is None or len(result.boxes) == 0:
        return out
    boxes = result.boxes.xyxy.detach().cpu().numpy()
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        # 1이면 기존 방식, 2 이상이면 남은 target에 반복 공격을 적용한다.
...
def main():
    ...
    # detector는 고정하고, 학습된 Vanish 생성자와 FastSAM만 시연에 사용한다.
    ...
    # 초기 target은 명령행 인자로 받고, 실행 중에는 숫자 키로 바꿀 수 있다.
    target = args.target
        ...
        with tqdm(total=total if total > 0 else None, desc="targeted vanish frames") as pbar:
            while not stop:
                ...
                with torch.no_grad():
                    ...
                    # 원본 프레임에서 선택 클래스만 분리하고 공격 마스크를 만든다.
                    ...
                    if attack_enabled:
                        # 공격 후에도 target이 남으면 설정된 횟수만큼 같은 프레임에서 추가 Vanish를 수행한다.
                        ...
                        # OFF 상태에서는 원본을 그대로 통과시켜 ON/OFF 비교가 가능하게 한다.
                        adv = img
                ...
                target_adv = filter_result_by_classes(result_adv, class_ids)
                # 전체 탐지 변화와 target class 변화는 별도로 기록한다.
                ...
                rows.append(
                    {
                        # 프레임별 CSV는 시연 결과를 나중에 수치로 검토하기 위한 기록이다.
                        "frame": frame_idx,
                        "target": target,
